...
if __name__ == "__main__":
    ...
    SETTINGS.grid(row=5, column=3, ipadx=30)

    L1 = Label(gui, text="Enter the delay")
    L1.grid(row=6, column=3, ipadx=30)
    DELAY = Entry(gui, bd=5)
    DELAY.grid(row=6, column=4, ipadx=20)

    L2 = Label(gui, text="Enter the key value to be pressed")
    L2.grid(row=7, column=3, ipadx=30)
    KEY_VALUE = Entry(gui, bd=5)
    KEY_VALUE.grid(row=7, column=4, ipadx=20)

    L3 = Label(gui, text="Enter number of times the key to be fired")
    L3.grid(row=8, column=3, ipadx=30)
    NUM = Entry(gui, bd=5)
    NUM.grid(row=8, column=4, ipadx=20)

    FIRE = Button(gui, text=' Fire ', fg='black', bg='green',
                      command=lambda: delay(), height=2, width=7)

    FIRE.grid(row=9, column=3 )
