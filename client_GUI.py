...
if __name__ == "__main__":
    ...
    sec = input('Enter the time delay.'
                'The maximum delay is 15 minutes and minimum delay is 1 sec\n')
    n = input('Enter the key number to be pressed.\n')
    p = input('Enter the number of times the key to be pressed.\n')
    x = int(p)
    startTime = time.time()
    if x > 0:
        for i in range(0, x):
